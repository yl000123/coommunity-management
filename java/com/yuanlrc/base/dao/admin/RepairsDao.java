...
import com.yuanlrc.base.entity.admin.Emp;
...
public interface RepairsDao extends JpaRepository<Repairs, Long> {
    ...
    @Query(value = "select suoyou.all_id\n" +
            ...
            "FROM ylrc_emp c , (SELECT b.building_id,a.emp_type FROM `ylrc_repairs` a , ylrc_dormitory b WHERE a.dormitory_id=b.id and a.id= :id) d \n" +
            ...
            "FROM ylrc_repairs f, \n" +
            "(SELECT c.id FROM ylrc_emp c , (SELECT b.building_id,a.emp_type FROM `ylrc_repairs` a , ylrc_dormitory b WHERE a.dormitory_id=b.id and a.id= :id) d \n" +
    ...
    @Query(value = "SELECT z.emp_id \n" +
            ...
            "FROM ylrc_repairs f, \n" +
            "(SELECT c.id FROM ylrc_emp c , (SELECT b.building_id,a.emp_type FROM `ylrc_repairs` a , ylrc_dormitory b WHERE a.dormitory_id=b.id and a.id=?) d \n" +
            "WHERE c.building_bno=d.building_id  and d.emp_type=c.emp_type) g\n" +
