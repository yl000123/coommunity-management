...
public interface StatisticsDao extends JpaRepository<Emp,Long> {
    ...
    //按照维修工类型统计任务量
    @Query(value = "SELECT emp_type , count(emp_type) FROM ylrc_repairs GROUP BY emp_type ", nativeQuery = true)
     List<Object> countByEmpType();

    //按照维修工统计任务量
    @Query(value = "SELECT emp_id, count(emp_id) FROM ylrc_repairs GROUP BY emp_id", nativeQuery = true)
